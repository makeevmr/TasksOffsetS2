                ...
                if (!command.empty()) {
                    ...
                    if (found_operator) {
                        ...
                        if (('a' <= command[0] && command[0] <= 'z') || ('A' <= command[0] && command[0] <= 'Z')) {
                            ...
                            matrix_stack.push(std::move(BigIntFractionMatrix(MatrixSize{1, 1})));
                            if (command.size() < 10) {
                                matrix_stack.top().fillScalar(
                                    std::move(BigIntegerFraction(IntFraction{std::stoi(command), 1})));
                            } else {
                                matrix_stack.top().fillScalar(
                                    std::move(BigIntegerFraction(StringFraction{command, "1"})));
                            }
                        }
                    }
                    command = "";
