...
BigInteger::BigInteger(int number) : size_(1), negative_(false) {
    if (number < 0) {
        negative_ = true;
        number *= -1;
    }
...
BigInteger::BigInteger(BigInteger &&other) noexcept: size_(0), ul_digit_number_(nullptr), negative_(false) {
    this->swap(other);
}
...
BigInteger &BigInteger::operator*=(const BigInteger &other) {
    ...
    for (int i = 0; i < other.size_; ++i) {
        ...
        for (int j = 0; j < this->size_; ++j) {
            uli_multiply(this->ul_digit_number_[j], other.ul_digit_number_[i], result, carry, separator);
...
const unsigned long int *BigInteger::get_data() const {
    return this->ul_digit_number_;
}
...
BigInteger operator+(const BigInteger &right_big_int) {
    return BigInteger(right_big_int);
}
...
unsigned long int lo_bits(unsigned long int x, int separator) {
    return ((1ULL << separator / 2) - 1) & x;
}

unsigned long int hi_bits(unsigned long int x, int separator) {
    return x >> separator / 2;
}

// calculate single ulls multiply with overflow handling
void uli_multiply(unsigned long int a, unsigned long int b,
                  unsigned long int &result, unsigned long int &carry, int separator) {
    ...
    result = s1 << separator / 2 | s0;
    carry = s3 << separator / 2 | s2;
}
